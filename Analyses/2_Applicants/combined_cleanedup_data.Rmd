...
Steps to fix this:
  ...
  3. dog_apps.csv - convert zip code from integer type to character type so that we can categorize them
...
Apps data needs to be cleaned up: 
  * Removed columns: STATEFP,COUNTYFP,TRACTCE,GEOID,NAME,NAMELSAD,MTFCC,FUNCSTAT,ALAND,AWATER,INTPTLAT,INTPTLON
  * Take absolute value of negative numbers
  * Any values > 15 for adults_in_home and children_in_home are updated N/A
  * ideal_adoption_timeline: consolidated next-few-weeks to few-weeks
  * all_household_agree: consolidated using the function clean_household_agree
  * home_owner and home_pet_policy: make factor
  * experience: cleaned using function above
  * City: cleaned to reduce same cities with different spellings
