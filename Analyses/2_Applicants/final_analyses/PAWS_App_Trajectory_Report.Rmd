...
```{r, echo=FALSE}
masterapps_20190324 %>% filter(outcome_type=="Adoption" & (!is.na(children_in_home))) %>%
...
cat_apps <- readr::read_csv(file = "Apps_Data/cat_apps.csv" ) %>%
  mutate(animal_type = "cat")

dog_apps <- readr::read_csv(file = "Apps_Data/dog_apps.csv") %>% 
