...
Below are visualizations that illustrate the applicants' characteristics (e.g. allergies, budget, home pet policy, etc.) for applications that were denied. We only have data for 12 denied applications so the analysis is limited. Hopefully with more data, we would be able to compare the denied applications to the adopted ones.<br><br>
Key takeaways:
* No known allergies for applicants
...
g2 <- ggplot(df_denied, aes(x=home_pet_policy)) + 
  ...
  labs(fill = "Animal Type") +
  coord_cartesian(ylim=c(0, 10)) +
  theme(axis.text.x = element_text(angle=50, vjust=0.5), axis.title.y = element_blank())
...
There were 133 applications that were red flagged. 129 of those have not yet resulted in an adoption or are still being procesed. Since the data set for the applications is from the end of 2018, many of the applications are still in progress. Two of the applications that were flagged were denied but that does not mean that the rest are going to result in adoption. We do not have the final status of many applications so we cannot conclude what happened to the red flagged applications. 

Below is a visualization that shows the last updated status for applications that were red flagged.
