...
extractFromString = function(str1) {
  ...
  #str(str1)
  #print(str1)
   if(!is.na(str1) & is.character(str1)) {
     if(str_detect(str1,",")) {
       sl = str_split(str1,",")
       #print("found comma")
     } else {
    ...
    #print(last)
    return (last) #(list(l,last))
   }
  return (NULL) #(list(NULL,NULL))
}
str1 = "checks, need to see id, need info"
ret=extractFromString(str1)
#ret[[1]]
#ret[[2]]
str(ret)
...
#cards %>% mutate(l=unlist(sapply(label_names, FUN=function(x) extractFromString(x)))) %>% View()
#cards %>% mutate (ld=dummy(label_names))
#cards %>% mutate(l=map(label_names,extractFromString))
cards
