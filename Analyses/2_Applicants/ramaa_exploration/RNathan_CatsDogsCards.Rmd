...
cards %>% filter(!dueComplete & due>dateLastActivity) %>% select(due,dateLastActivity,lastLabel)
```
Observation: There are 7 cases when dueComplete is TRUE and due date is later then dateLastActivity. There are 51 cases where dueCompelte is FALSE and the due date is later than dateLastActivity
