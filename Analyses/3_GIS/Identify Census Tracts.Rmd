...
We have `r round(coverage,2)`% coverage.  We have `rcoverage_table[1]+coverage_table[2]` total addresses, of which `r coverage_table[1]` have a GEOID (Census Tract), and `r coverage_table[2]` are missing that data.

## Get Census Data

Now we have to think about what Census Bureau data to obtain.  Large metropolitan areas get attention from the American Community Survey, which happens with much greater frequency than the decennial (every ten years) census, so it's more up to date.  But, it's also an estimate, as it's a survey that obtains data from a selection of the population.  We know that some of our adopters do *not* come from the Philly area and in fact live in rural areas.  So the ACS might not be the right choice.
...
cat_applicants <- merge(x = cat_applicants, 
                        y = adopter_economic_characteristics,
                        by = "GEOID",
                        all.x = TRUE)
