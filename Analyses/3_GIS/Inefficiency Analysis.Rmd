...
- For applications that have a decision (see below for labels that indicate this), the last recorded checklist item is checked off 7 days or less from the date of submission
- For applications that don't have a decision (processing is ongoing), the submission date is more than 2 weeks prior to 12/31/2018 
...
Next, we're going to label those applications as efficient where the processing took 14 days or less. 

```{r}
dog.appsandcards$efficient <- ifelse(dog.appsandcards$processing.time>14,0,1)
cat.appsandcards$efficient <- ifelse(cat.appsandcards$processing.time>14,0,1)
```
...
We have an adjusted R2 of `r summary(model)$adj.r.squared`. Let's plot the overall lm residuals on a map.
...
So if I'm understanding this correctly, the model fits better in the far outskirts than in CC or northeast. Let's plot the predictors that were significant.
...
# HC02_EST_VC20_comp = Percent; Estimate; TYPE OF INTERNET SUBSCRIPTIONS - With an Internet subscription: - Broadband of any type - Cellular data plan - Cellular data plan with no other type of Internet subscription
# HC02_EST_VC05_comp = Percent; Estimate; TYPES OF COMPUTER - Has one or more types of computing devices: - Desktop or laptop
# HC02_EST_VC16_educ = Percent; Estimate; Population 3 to 4 years - 3 to 4 year olds enrolled in school
# HC01_EST_VC22_educ = Total; Estimate; Population 10 to 14 years - 10 to 14 year olds enrolled in school
# HC02_EST_VC22_comp = Percent; Estimate; TYPE OF INTERNET SUBSCRIPTIONS - With an Internet subscription: - Broadband of any type - Satellite Internet service
# HC02_EST_VC05_educ = Percent; Estimate; Elementary: grade 1 to grade 4
