...
#   theme_bw() + ylab("Average") + xlab("Is there a photo?")
...
#             retweet = mean(retweet_count, rm.na = T)) %>% 
#   gather(metric, average, favorite:retweet) %>% 
#   ggplot(aes(x=animal_type, y=average, fill = metric)) +
#   geom_col(position = "dodge") + 
#   theme_bw() + 
#   ggtitle("Tweets with photos of cats \nare liked and retweeted more")
...
#   geom_jitter(aes(color=animal_type), alpha = 0.75, height = 0.1, width = 0.1) + 
#   theme_bw() + 
...
# tweets %>% 
#   filter(retweet_count < 200) %>% 
#   ggplot(aes(x=predict, y=retweet_count)) +
#   geom_point(aes(color=animal_type), alpha = 0.5) + 
#   theme_bw() + 
#   geom_smooth(method = "lm") +
#   ylim(c(0,50)) + 
#   ggtitle("Tweets with photos of cats \nare retweeted more")
